...
def test_todo_filtering_and_sorting(client, auth_headers):
    client.post(
        "/api/todos", json={"title": "b_task", "completed": True}, headers=auth_headers
    )
    client.post(
        "/api/todos", json={"title": "a_task", "completed": False}, headers=auth_headers
    )
...
def test_update_todo(client, auth_headers):
    post_resp = client.post(
        "/api/todos", json={"title": "old_title"}, headers=auth_headers
    )
    todo_id = post_resp.get_json()["id"]

    response = client.put(
        f"/api/todos/{todo_id}",
        json={"title": "new_title", "completed": True},
        headers=auth_headers,
    )
    assert response.status_code == 200
    data = response.get_json()
...
def test_delete_todo(client, auth_headers):
    post_resp = client.post(
        "/api/todos", json={"title": "task_to_del"}, headers=auth_headers
    )
    todo_id = post_resp.get_json()["id"]

    assert (
        client.delete(f"/api/todos/{todo_id}", headers=auth_headers).status_code == 204
    )
