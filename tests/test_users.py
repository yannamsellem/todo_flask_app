...
def test_update_own_user(client):
    ...
    client.post(
        "/api/auth/register",
        json={"username": "upduser", "email": "upd@ex.com", "password": "password123"},
    )
    login_resp = client.post(
        "/api/auth/login", json={"username": "upduser", "password": "password123"}
    )
    ...
    response = client.put(
        f"/api/users/{user_id}", json={"username": "newname"}, headers=headers
    )
    assert response.status_code == 200
...
def test_delete_own_user(client):
    client.post(
        "/api/auth/register",
        json={"username": "deluser", "email": "del@ex.com", "password": "password123"},
    )
    login_resp = client.post(
        "/api/auth/login", json={"username": "deluser", "password": "password123"}
    )
    token = login_resp.get_json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
